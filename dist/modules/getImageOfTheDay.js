...
function getImageOfTheDay() {
  ...
  if (dateInLocalStorage !== today || isInLocalStorage === null) {
    ...
    getWordOfTheDay().then(response => response.json()).then(({
      word
    }) => {
      fetch(`https://api.unsplash.com/search/photos?query=${word}&client_id=${unsplashApiKey}&orientation=landscape`).then(response => response.json()).then(data => {
        if (data.total === 0) {
